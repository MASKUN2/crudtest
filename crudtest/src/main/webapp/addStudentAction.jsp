...
<%
	...
	String[] alias = {"studentName", "studentGender", "studentBirth",}; // 조회할 파라메터를 명세하기
	for(int i = 0; i < alias.length; i += 1){ // 조회한 리퀘스트 파라메터값이 널이거나 공란인경우 ?fail=leak 로 이전페이지로 보냄
		if(request.getParameter(alias[i]) == null || request.getParameter(alias[i]) == ""){
			response.sendRedirect(request.getContextPath()+"/addStudentForm.jsp?fail=leak");
			return;
		}
	}
	...
	//빈 인자 쿼리를 지정
	String sql = ("INSERT INTO student(student_name, student_gender, student_birth, team_no, createdate, updatedate) VALUES (?,?,?,?, now(), now())");
	...
	if(row == 1) {
		...
		response.sendRedirect(request.getContextPath()+"/studentList.jsp?"); // 성공하면 이전페이지로
	}else{
		System.out.println("실패");
		response.sendRedirect(request.getContextPath()+"/addStudentForm.jsp?fail=true"); //이전 폼으로 이동시킴
	}	
%>
